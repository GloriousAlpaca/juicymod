...
public class TankTile extends TileEntity {
	...
	/*
	 * public TankControllerTile searchController(Vector<BlockPos> marked) { if
	 * (!this.getWorld().isRemote()) { // Does this block already have a controller?
	 * if (controller != null) return controller; // Mark this block as read
	 * marked.add(this.pos); // Go through the neighbours for (int i = 0; i < 6;
	 * i++) { BlockPos cpos = this.pos.add(neighbours[i]); // Is this neighbour part
	 * of the Tank? if (world.getBlockState(cpos).getBlock() instanceof TankBlock &&
	 * !marked.contains(cpos)) { TileEntity current = world.getTileEntity(cpos); //
	 * If the block is not a controller check the next block if (current instanceof
	 * TankControllerTile) { return (TankControllerTile) current; } else{
	 * //Juicy.LOGGER.info("OPENING: "+cpos.toString()); TankControllerTile cntrllr
	 * = ((TankTile) current).searchController(marked); if(cntrllr !=null) return
	 * cntrllr; } } } } return null; }
	 */
